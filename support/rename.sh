...
filesToModify=$(grep -riIl 'proyect-x' --exclude='rename.sh' $appRoot/*)
filesToRename=$(find "${appRoot}/ios" "${appRoot}/android" -type f -ipath '*proyect-x*')

# replace strings in files
for fileToModify in $filesToModify; do
  sed -i.bak "s/proyect-x/${newAppName}/g;s/proyect-x/${newLowerCaseName}/g" $fileToModify
done
...
for fileToRename in $filesToRename; do
  newName=$(echo $fileToRename | sed "s/proyect-x/$newAppName/g;s/proyect-x/$newLowerCaseName/g")
  mkdir -p $(dirname "$newName") && $mvCmd "$fileToRename" "$newName"
done

# remove leftover empty directories
rmdir -p $(find "$appRoot" -ipath '*proyect-x*' -type d) 2>/dev/null
